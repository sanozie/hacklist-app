...
exports.redirect = functions.https.onRequest((req, res) => {
    ...
    functions.logger.log('Token req', req)
    functions.logger.log('Token req headers cookie', req.headers.cookie)
    cookieParser()(req, res, () => {
        const cookie = convertCookie(req.headers.cookie)

        const state = cookie || crypto.randomBytes(20).toString('hex');
        functions.logger.log('state', state)
...
exports.token = functions.https.onRequest((req, res) => {
    const Linkedin = linkedInClient();
    functions.logger.log('Start req', req)
    functions.logger.log('Start req cookie', req.headers.cookie)
    try {
        const cookie = convertCookie(req.headers.cookie)
        return cookieParser()(req, res, () => {
            ...
            functions.logger.log('Received verification state:', cookie);
            functions.logger.log(cookie)

            Linkedin.auth.authorize(OAUTH_SCOPES, cookie); // Makes sure the state parameter is set
            ...
            Linkedin.auth.getAccessToken(res, req.query.code, req.query.state, (error, results) => {
                ...
                functions.logger.log('Received Access Token:', linkedInAccessToken);
                ...
                linkedin.people.me(async (error, profileObject) => {
                    ...
                    request(emailEndpoint, function (error, response, emailObject) {
                        ...
                        functions.logger.log('body:', emailObject); // Print the HTML for the Google homepage.


                        if (error) {
                            functions.logger.log("LinkedIn GET: ", error);
                            return
                        }

                        let emailAddress = getEmailAddress(emailObject);
                        functions.logger.log("emailAddress: ", emailAddress);
                        ...
                        functions.logger.log("==> Fetch photo");

                        request(photoEndpoint, async function (error, response, photoObject) {

                            console.error('error:', error); // Print the error if one occurred
                            functions.logger.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
                            functions.logger.log('body:', photoObject); // Print the HTML for the Google homepage.
                            ...
                            functions.logger.log("photoUrl: ", photoUrl);
